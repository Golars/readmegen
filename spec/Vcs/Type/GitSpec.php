...
class GitSpec extends ObjectBehavior
{
    ...
    function it_should_add_options_and_arguments_to_the_command(Shell $shell)
    {
        ...
        $this->setOptions(['x', 'y']);
        $this->setArguments(['foo' => 'bar', 'baz' => 'wat', 'from' => '1.0']);
    ...
    function it_should_properly_include_the_from_and_to_arguments()
    {
        ...
        $this->setArguments(['from' => '3.4.5', 'foo' => 'bar']);
        $this->getCommand()->shouldReturn('git log --pretty=format:"%s' . Git::MSG_SEPARATOR
                                          . '%b" 3.4.5..HEAD --x --y');

        $this->setArguments(['from' => '3.4.5', 'foo' => 'bar', 'to' => '4.0']);
        $this->getCommand()->shouldReturn('git log --pretty=format:"%s' . Git::MSG_SEPARATOR
                                          . '%b" 3.4.5..4.0 --x --y');
    }
    ...
    function it_returns_the_date_of_the_commit(Shell $shell)
    {
        ...
        $this->setArguments(['from' => '1.0', 'to' => '3f04264']);
